(() => {
  ...
  var Room = class {
     ...
     */
    constructor(doc2, provider2, name, key) {
      this.peerId = uuidv4();
      this.doc = doc2;
      this.awareness = provider2.awareness;
  ...
  var openRoom = (doc2, provider2, name, key) => {
    if (rooms.has(name)) {
      throw create3(`A Yjs Doc connected to room "${name}" already exists!`);
    }
    const room = new Room(doc2, provider2, name, key);
    rooms.set(
      name,
      /** @type {Room} */
      room
  ...
  var emitStatus = (provider2) => {
    provider2.emit("status", [{
      connected: provider2.connected
    }]);
  };
  ...
  var provider = new WebrtcProvider(ROOM_ID, ydoc);
  console.log("RoomID: ", ROOM_ID);
  console.log("provider: ", provider);
  var yText = ydoc.getText("codemirror");
  console.log("yText: ", yText);
  var binding = new CodemirrorBinding(yText, editor, provider.awareness);
